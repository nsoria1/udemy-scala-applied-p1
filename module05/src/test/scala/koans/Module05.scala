...
class Module05 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Multiply numbers") {
    ...
    def mult(n: Int): Int = multiplier * n
  ...
  test ("Multiply variable number of arguments") {
    ...
    def multiplyDoubles(num: Double*): Double = num.product
  ...
  test ("Recurse with varargs") {
    ...
    def listOfLists(arg: String*): List[List[String]] = {
      if (arg.isEmpty) Nil
      else arg.toList :: listOfLists(arg.tail: _*)
    }
  ...
  test ("Opposites matcher") {
    ...
    def oppositeOf(item: String): String = item match {
      case "North" => "South"
      case "Hot" => "Cold"
      case "Cool" => "Square"
      case v => s"Not ${v}"
